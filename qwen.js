...
const use_chat = (...args) => Promise.all(args.map(i => (i.constructor === String || i.constructor === Number) ? user_say(i) : i))
    .then(input => {
        ...
        const hash = md5([messages, functions, options].map(i => JSON.stringify(i)).join('\n'));
        ...
        const req = ['max_tokens', 'temperature', 'tool_choice']
        ...
        return fs.readFile(filename, 'utf-8')
            ...
            .then(
                /**
                 * @param {{choices: Array<{message: {role: string, content: string,tool_calls:Array<{function:Object,id:string,type:string}>}, index?: number, finish_reason?: string}>}} res
                 */
                async res => {
                    const called = [];
                    for (const item of res.choices) {
                        if (item.finish_reason === 'tool_calls') {
                            ...
                            for (const req of item.message.tool_calls) {
                                ...
                                called.push(await Promise.resolve(call($require(path)[name], options.inject, JSON.parse(req.function.arguments)))
                                    .catch(e => e.toString())
                                    .then(res => ({
                                        tool_call_id: req.id,
                                        index: req.index,
...
/**
 * 递归读取指定目���下的所有函数定义（需配合 babel 解析）。
 * @param {string} path - 目录路径。
 * @param {Object} inject - 注入的变量对象。
 * @returns {Promise<Array>} 函数描述对象数组。
 */
const use_functions = (path, inject = null) => fs.readdir(path, {withFileTypes: true, recursive: true})
    .then(async rs => {
        const [key] = Object.getOwnPropertySymbols(rs[0]).filter(s => s.description === 'type');

        const dirs = new Set(), libs = [];
        for (const {name, path, [key]: type} of rs.sort((a, b) => b[key] - a[key])) { //排序后再处理，文件夹优先
            const filename = path.slice(0, -join(path).length) + join(path, name);
            if (type === 2) dirs.add(filename);
            else {
                const dir = join(path);
                if (dirs.has(dir) && /^(main|index|export|expose)\.[a-z]{0,3}js$/i.test(name)) {
                    libs.push({
                        standalone: false,
                        filename,
                        dir,
        ...
        for (const {filename, standalone, dir} of libs) {
            ...
            for (const node of babel.parse(await fs.readFile(filename, {encoding: 'utf-8'}), {
                ...
                for (const fn of right?.properties ?? []) {
                    ...
                    const name = `fc_${hash}_${fn.key.name}`,
                    ...
                    result.push({
                        ...
                        [field]: {
                            ...
                            call: (fn, env = inject, arg) => {
                                console.log('Call', path, name, arg, env);
