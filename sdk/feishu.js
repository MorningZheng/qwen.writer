...
module.exports = (app_id, app_secret, temp_dir, expired_delay = 0.1 * 60 * 1000) => {
	 ...
	 * @returns {{chat_id:string, get_info:function, get_menu:function, get_config:function, send_msg:function,send_card:function,update_card:function,use_card:function,prev_msg:function, get_messages:function,get_tabs:function}}
	 */
	const use_chat = chat_id => {
		...
		const expose = {
			...
			send_card: data => expose.send_msg(data, 'interactive'),
			update_card: (message_id, data) => use_bearer({
				path: {message_id},
				data: {content: data.constructor === String ? data : JSON.stringify(data)},
			}).with(client.im.v1.message.patch),

			use_card(title, summary) {
				let card_id = null;
				...
				const card = {
					...
					config: {
						...
						// 	content: "示例标题", // 自定义摘要信息。如果开启了流式更新模式，该参数将默认为“生成中”。
						// },
						enable_forward: false, // 是否支持转发卡片。默认值为 true。
				...
				const reset = (title, summary) => {
					if (title) handler.set_title(title);
				...
				const handler = {
					commit() {
						if (!card_id) return expose.send_msg(card, 'interactive').then(rs => {
							card_id = rs.message_id;
							return rs;
						});
					...
					set_title(content) {
						card.header.title = {tag: "plain_text", content};
						return handler;
					},
					set_summary(content) {
			...
			}).with(client.im.v1.message.list).then(r => JSON.parse(r.items?.[0]?.body?.content ?? '{}')),
		};
